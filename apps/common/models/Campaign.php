...
class Campaign extends ActiveRecord
{
	 ...
	 * @since 1.9.5
	 */
    public function postponeBecauseCustomerReachedQuota()
    {
    	if (empty($this->customer_id) || empty($this->customer)) {
    		return false;
	    }
	    ...
	    $save = $this->saveAttributes(array(
    	...
    	if (!$options->get($optionsKey)) {
		    ...
		    $message->message_translation_params = array(
			    '{campaign}' => CHtml::link($this->name, $options->get('system.urls.customer_absolute_url') . 'campaigns/' . $this->campaign_uid . '/overview'),
			    '{date}'     => $this->getSendAt()
		    );
		    $message->save();
	    }
